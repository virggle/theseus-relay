...
## 约束
用户下达的、约束你行为方式的通用指令（例如「改文件前先把改法给我看」「回复只用中文」）——逐条列出，一条都不许丢
## 用户画像
沟通风格、背景、在意什么
## 开放问题
...
export function buildSystemPrompt(agentId, prevHandoff) {
  const prev = prevHandoff
    ...
    : '【工作简报】你是第一位助手，没有简报。请自然回应用户，并按下方固定格式写 handoff（各节内容可以简短，但六节一个都不能少）。';
...
- 简报「决策」与「约束」两节只增不删：已确定的结论、被否掉的方案及理由，一条都不许丢；用户下达的通用约束（"改文件前先把改法给我看""回复只用中文"这类）同样一条都不许丢——它不隶属于任何单个任务，最容易被后棒当成无关内容清掉。
- 严禁元注释和占位符：不许写「（保留全部旧结论）」「同上」「略」这类缩写——下一任助手看不到旧简报的原文，占位符等于销毁信息。「决策」「约束」与「用户画像」必须逐条完整写出，哪怕与上一份简报一字不差。
- 「约束」只增不删；「用户画像」合并更新；「开放问题」清旧加新；「副作用」如实记录；总长 ${BRIEF_BUDGET} 字以内，优先级：决策与约束 > 副作用 > 用户画像 > 开放问题 > 进展细节。

你的简报会被基底机械校验（不是人看，是脚本判）：六节齐全、无占位符、${BRIEF_BUDGET} 字内、决策/约束/用户画像条目不得比上一份少。校验不过会被拒收并要求你重发，所以一次写对更省事。

【简报固定格式——每一棒都必须遵守，六节缺一不可，节标题原样保留】
${HANDOFF_FORMAT}
...
{"reply":"回复用户的内容","handoff":"按上述六节格式更新的工作简报"}
